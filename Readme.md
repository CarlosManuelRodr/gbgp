...
Despite the name the idea is dead simple. Take a problem that can be solved via a domain specific language, encode the language into a grammar that generates valid solutions, set-up a target function to optimize and let the system approximate a solution by applying the rules of natural selection.
...
```
and ask you to tell me which expression f(x,y) generated them. A simple solution would be to just go to WolframAlpha, write those points in the query and let it find the answer for you. But sadly, doing that is lame and you won't be able to brag about it. So alternatively lets use gbgp!

Firstly define a grammar for arithmetic operations. Here we set a special set of rules called "production rules":
...
Each production rule is a recipe for generating children terms for a non-terminal term. Non-Terminals are denoted by the UPPER_CASE name and they are terms that have other terms as children. The lowerCase terms are called Terminal terms and they have no children and only contain a value.
